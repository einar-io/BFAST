
-- ==
-- entry: bfast-naive bfast-opt bfast-opt-alt
-- compiled input @ ../../data/sahara.in.gz
-- output @ ../../data/sahara.out.gz

-- -- ==
-- -- entry: sanity
-- -- compiled input @ ../../data/sahara.in.gz
-- -- output { 3i32 228i32 12.0f32 0.25f32 1.736126f32 67968i64 414i64 }

-- ==
-- entry: bfast-1
-- compiled input @ ../../data/sahara-all.in.gz
-- output @ ../../data/sahara-1.out.gz

-- ==
-- entry: bfast-2 bfast-2-tiled bfast-2-shr
-- compiled input @ ../../data/sahara-all.in.gz
-- output @ ../../data/sahara-2.out.gz

-- ==
-- entry: bfast-3
-- compiled input @ ../../data/sahara-all.in.gz
-- output @ ../../data/sahara-3.out.gz

-- ==
-- entry: bfast-4a bfast-4a-flipped bfast-4a-tiled
-- compiled input @ ../../data/sahara-all.in.gz
-- output @ ../../data/sahara-4a.out.gz

-- ==
-- entry: bfast-4b
-- compiled input @ ../../data/sahara-all.in.gz
-- output @ ../../data/sahara-4b.out.gz

-- ==
-- entry: bfast-4c bfast-4c-flipped bfast-4c-tiled
-- compiled input @ ../../data/sahara-all.in.gz
-- output @ ../../data/sahara-4c.out.gz

-- ==
-- entry: bfast-5
-- compiled input @ ../../data/sahara-all.in.gz
-- output @ ../../data/sahara-5.out.gz

-- ==
-- entry: bfast-6 bfast-6-reuse
-- compiled input @ ../../data/sahara-all.in.gz
-- output @ ../../data/sahara-6.out.gz

-- ==
-- entry: bfast-7a
-- compiled input @ ../../data/sahara-all.in.gz
-- output @ ../../data/sahara-7a.out.gz

-- ==
-- entry: bfast-7b
-- compiled input @ ../../data/sahara-all.in.gz
-- output @ ../../data/sahara-7b.out.gz

-- ==
-- entry: bfast-8 bfast-8-reuse bfast-8-opt2 bfast-8-less-shr
-- compiled input @ ../../data/sahara-all.in.gz
-- output @ ../../data/sahara-8.out.gz

